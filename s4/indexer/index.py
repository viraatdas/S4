...
class DocumentIndex:
    ...
    def __init__(self, index_id: str = "default"):
        ...
        self.index_path = config.INDEX_STORAGE_PATH / f"{index_id}.faiss"
        self.metadata_path = config.INDEX_STORAGE_PATH / f"{index_id}.json"
    ...
    def search(
        ...
        """
        # Define filter function if filtering by file_id
        filter_fn = None
        if filter_by_file_id:
            filter_fn = lambda metadata: metadata.get('file_id') == filter_by_file_id
